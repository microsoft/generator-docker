...
describe('.NET RTM project file creation (Web project)', function () {
    before(function (done) {
        ...
        .withPrompts({ projectType: 'dotnet', isDotNetWeb: true, baseImageName: 'dotnet:1.0.0-preview2-sdk', imageName: 'testimagename' })
        .on('end', done);
    });
...
describe('.NET RTM project file creation (Console project)', function () {
    before(function (done) {
        helpers.run(path.join(__dirname, '../generators/app'))
        .inTmpDir(function (dir) {
            createTestProjectJson(dir);
        ...
        .withPrompts({ projectType: 'dotnet', isDotNetWeb: false, baseImageName: 'dotnet:1.0.0-preview2-sdk', imageName: 'testimagename' })
        .on('end', done);
    });
